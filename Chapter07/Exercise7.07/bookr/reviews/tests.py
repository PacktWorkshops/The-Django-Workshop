...
class Exercise7Test(LiveServerTestCase):
    ...
    def test_media_example_upload(self):
        ...
        try:
            c = Client()
            with open(os.path.join(settings.BASE_DIR, 'static', logo_filename), 'rb') as logo_fp:
                with open(os.path.join(settings.BASE_DIR, 'static', css_filename), 'rb') as css_fp:
                    resp = c.post('/media-example/', {'file_field': css_fp, 'image_field': logo_fp})

            self.assertEquals(resp.status_code, 200)
            self.assertIn(b'<img src="/media/images/logo.png">', resp.content)
